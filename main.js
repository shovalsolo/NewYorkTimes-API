var gulp = require('gulp');var concat = require('gulp-concat');var uglifycss = require('gulp-uglifycss');var uglify = require('gulp-uglify');var browserSync = require('browser-sync').create();var reload = browserSync.reload;var sass = require('gulp-sass');var watchSass = require("gulp-watch-sass") var plumber = require('gulp-plumber');gulp.task('all',function(){gulp.src('*.css') .pipe(concat('main.css')) .pipe(uglifycss()) .pipe(gulp.dest(''));gulp.src('*.js') .pipe(concat('main.js')) .pipe(uglifycss()) .pipe(gulp.dest(''))});gulp.task('css',function(){gulp.src('*.css') .pipe(concat('main.css')) .pipe(uglifycss()) .pipe(gulp.dest(''))});gulp.task('js',function(){gulp.src('*.js') .pipe(concat('main.js')) .pipe(uglifycss()) .pipe(gulp.dest(''))});gulp.task('browser-sync',function(){browserSync.init({server:{baseDir:"./"}});gulp.watch(["*.html","dist/css/style.css","dist/js/sctipt.js"]).on("change",reload)});gulp.task('sass',function(){gulp.src('*.scss') .pipe(sass()) .pipe(gulp.dest('./css'))});gulp.task('to-sass',function(){gulp.src('*.scss') .pipe(plumber()) .pipe(sass()) .pipe(concat('style.css')) .pipe(uglifycss()) .pipe(gulp.dest('.'))});gulp.task('sass-2-css',function(){gulp.src('./comp/scss/*.scss')
  .pipe(plumber())
  .pipe(sass())
...
gulp.task('unify-js', function() {
    gulp.src('./comp/js gulp.task("css-watch",function(){gulp.watch(["./comp/scss/*.scss"],["sass-2-css"]);
});
